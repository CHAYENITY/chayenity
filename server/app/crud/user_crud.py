...
async def create_user_profile(
    db: AsyncSession, user_id: UUID, profile_setup: UserProfileSetup
) -> User:
    # Get the user
    ...
    if profile_setup.address:
        ...
        if profile_setup.address.latitude and profile_setup.address.longitude:
            point = WKTElement(
                f"POINT({profile_setup.address.longitude} {profile_setup.address.latitude})",
                srid=4326,
            )
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=400, detail="Failed to create user profile") from exc
